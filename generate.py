...
OUTPUT_FILE = "docs/jamie_oncall.ics"
...
def is_jamie_oncall(event_text):
    summary = get_summary(event_text).lower()
    return "jamie" in summary and ("on call" in summary or "oncall" in summary)

def is_jamie_vacation(event_text):
    summary = get_summary(event_text).lower()
    return "jamie" in summary and "vacation" in summary
...
def build_jamie_ics(events):
    output_events = []
    for event_text in events:
        if is_jamie_oncall(event_text):
            process_event(event_text, output_events, is_vacation=False)
        elif is_jamie_vacation(event_text):
            process_event(event_text, output_events, is_vacation=True)

    ics_lines = [
        "BEGIN:VCALENDAR",
        "VERSION:2.0",
        "PRODID:-//Jamie On Call//EN",
        "CALNAME:Jamie Schedule",
        "X-WR-CALNAME:Jamie Schedule",
        "X-WR-CALDESC:Jamie on-call and vacation schedule from RBT Work Calendar",
        "CALSCALE:GREGORIAN",
        "METHOD:PUBLISH",
    ]
...
def main():
    ...
    oncall_events = [e for e in events if is_jamie_oncall(e)]
    vacation_events = [e for e in events if is_jamie_vacation(e)]
    print(f"  Found {len(oncall_events)} Jamie on-call events (before expansion)")
    print(f"  Found {len(vacation_events)} Jamie vacation events")

    print("Building ICS...")
    ics_content = build_jamie_ics(events)
